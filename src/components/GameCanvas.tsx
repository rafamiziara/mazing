...
import { Game, Dir, Mode, Status } from "@/lib/game";
...
export default function GameCanvas({ mode }: { mode: Mode }) {
  ...
  const seedLabel =
    mode === "daily" ? `daily:${dailyKey()}` : `journey:${Date.now().toString(36)}`;
  ...
  useEffect(() => {
    ...
    game.onEvent = (e) => {
      switch (e) {
        case "step": audio.step(); break;
        case "spark": audio.spark(); break;
        case "charge": audio.charge(); break;
        case "pulse": audio.pulse(); break;
        case "hit": audio.hit(); audio.resetCombo(); break;
        case "portal": audio.portal(); break;
        case "death": audio.death(); break;
        case "lowlight": audio.lowLight(); break;
        case "levelstart": audio.resetCombo(); break;
      }
          ...
          <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between p-4 sm:p-6">
            <div className="flex items-center gap-4">
              ...
              <div className="font-mono text-xs tracking-widest text-parchment/60 uppercase">
                {mode === "daily" ? (
                  <span className="text-portal">daily #{dailyNumber()}</span>
                ) : (
            ...
            <div className="text-right">
              <div className="font-mono text-lg font-medium text-parchment tabular-nums">
          ...
          <div className="pointer-events-none absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 p-4 sm:p-6">
            <div className="w-44 sm:w-56">
              <div className="mb-1.5 flex items-center justify-between font-mono text-[10px] tracking-widest uppercase">
                <span className={hud.light < 0.25 ? "animate-pulse text-thread" : "text-parchment/50"}>
                  light
                </span>
                <span className="text-parchment/50">
                  echo {"◆".repeat(Math.min(hud.pulses, 5))}{hud.pulses > 5 ? `+${hud.pulses - 5}` : ""}
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-night/80 backdrop-blur-sm">
          <div className="flex flex-col items-center gap-6 text-center">
            ...
            <p className="max-w-xs text-sm text-parchment/60">
              The maze waits. Your light, for now, does not burn down.
            </p>
            ...
            <p className="font-mono text-[11px] tracking-wider text-parchment/40">
              esc resume · m {muted ? "unmute audio" : "mute"}
            </p>
          </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-night/70 p-6 backdrop-blur-sm">
          <div className="flex w-full max-w-sm flex-col items-center gap-5 rounded-2xl border border-white/10 bg-night-high/90 p-8 text-center shadow-2xl shadow-black/60">
            ...
            <dl className="grid w-full grid-cols-3 gap-2 font-mono text-parchment/80">
              ...
              <div className="rounded-lg bg-white/5 p-3">
                <dt className="text-[9px] tracking-widest text-parchment/40 uppercase">
                  {mode === "journey" ? "level" : "sparks"}
                </dt>
                <dd className="mt-1 text-base tabular-nums">
                  {mode === "journey" ? hud.level : `${hud.sparksTaken}/${hud.sparksTotal}`}
                </dd>
              </div>
              <div className="rounded-lg bg-white/5 p-3">
