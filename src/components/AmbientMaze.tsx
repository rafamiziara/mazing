...
import { generateMaze, shortestPath, isOpen, N, S, E, W, Maze } from "@/lib/maze";
...
export default function AmbientMaze({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const rebuild = () => {
      ...
      const sx = Math.floor(rng() * cols), sy = Math.floor(rng() * rows);
      trail = [{ x: sx + 0.5, y: sy + 0.5 }];
      headX = sx + 0.5; headY = sy + 0.5;
      path = [];
      pathPos = 0;
    };
    rebuild();
    ...
    const pickNewPath = () => {
      const cx = Math.floor(headX), cy = Math.floor(headY);
      const tx = Math.floor(rng() * cols), ty = Math.floor(rng() * rows);
    ...
    const drawWalls = () => {
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          if (!isOpen(maze, x, y, N)) { ctx.moveTo(px, py); ctx.lineTo(px + cell, py); }
          if (!isOpen(maze, x, y, W)) { ctx.moveTo(px, py); ctx.lineTo(px, py + cell); }
          if (y === rows - 1 && !isOpen(maze, x, y, S)) { ctx.moveTo(px, py + cell); ctx.lineTo(px + cell, py + cell); }
          if (x === cols - 1 && !isOpen(maze, x, y, E)) { ctx.moveTo(px + cell, py); ctx.lineTo(px + cell, py + cell); }
        }
      }
      ctx.stroke();
    };
    ...
    const drawFrame = (t: number) => {
      ...
      const hx = headX * cell, hy = headY * cell;
    ...
    if (reduced) {
      ...
      drawFrame(0);
      const onResize = () => { rebuild(); pickNewPath(); drawFrame(0); };
    ...
    const onResize = () => { rebuild(); pickNewPath(); };
    window.addEventListener("resize", onResize);
    return () => {
