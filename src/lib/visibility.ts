 ...
 */
export function visibilityPolygon(ox: number, oy: number, segments: Segment[], radius: number): Point[] {
 ...
 */
function raySegment(ox: number, oy: number, dx: number, dy: number, s: Segment): number | null {
  const sx = s.bx - s.ax
  const sy = s.by - s.ay
