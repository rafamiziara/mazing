...
import { E, idx, isOpen, type Maze, N, S, W, wallSegments } from './maze'
import { type Point, pointInPolygon, visibilityPolygon } from './visibility'
...
export class Renderer {
  ...
  render(game: Game, t: number, dt: number) {
    ...
    // --- 1. background
    const bgGrad = ctx.createRadialGradient(this.w / 2, this.h / 2, 0, this.w / 2, this.h / 2, Math.max(this.w, this.h) * 0.75)
    ...
    const segs = wallSegments(
      s.maze,
      Math.floor(s.px - radius) - 1,
      Math.floor(s.py - radius) - 1,
      Math.ceil(s.px + radius) + 1,
      Math.ceil(s.py + radius) + 1
    )
    ...
    if (s.status !== 'over') this.drawPlayer(s, t)
  ...
  private wallPath(m: Maze, x0: number, y0: number, x1: number, y1: number) {
    ...
    for (let y = y0; y <= y1; y++) {
      for (let x = x0; x <= x1; x++) {
        ...
        if (y === m.rows - 1 && !isOpen(m, x, y, S)) {
          ctx.moveTo(px, py + c)
          ctx.lineTo(px + c, py + c)
        }
        if (x === m.cols - 1 && !isOpen(m, x, y, E)) {
          ctx.moveTo(px + c, py)
          ctx.lineTo(px + c, py + c)
        }
      }
  ...
  private drawMemory(s: GameState, x0: number, y0: number, x1: number, y1: number) {
    ...
    for (let y = y0; y <= y1; y++) {
      for (let x = x0; x <= x1; x++) {
        ...
        const px = this.sx(x),
          py = this.sy(y)
        if (!isOpen(s.maze, x, y, N)) {
          ctx.moveTo(px, py)
          ctx.lineTo(px + c, py)
        }
        if (!isOpen(s.maze, x, y, W)) {
          ctx.moveTo(px, py)
          ctx.lineTo(px, py + c)
        }
        if (!isOpen(s.maze, x, y, S)) {
          ctx.moveTo(px, py + c)
          ctx.lineTo(px + c, py + c)
        }
        if (!isOpen(s.maze, x, y, E)) {
          ctx.moveTo(px + c, py)
          ctx.lineTo(px + c, py + c)
        }
      }
  ...
  private drawLight(game: Game, s: GameState, t: number) {
    ...
    const flicker = this.reducedMotion ? 1 : 0.96 + 0.04 * this.noise[Math.floor(t * 9) % 512] + 0.02 * Math.sin(t * 17)
    const rPx = radius * c * flicker
    const px = this.sx(s.px),
      py = this.sy(s.py)
    ...
    for (let y = wy0; y <= wy1; y++) {
      for (let x = wx0; x <= wx1; x++) {
        ...
        if (!isOpen(s.maze, x, y, N)) {
          ctx.moveTo(pxc, pyc)
          ctx.lineTo(pxc + c, pyc)
        }
        if (!isOpen(s.maze, x, y, W)) {
          ctx.moveTo(pxc, pyc)
          ctx.lineTo(pxc, pyc + c)
        }
        if (!isOpen(s.maze, x, y, S)) {
          ctx.moveTo(pxc, pyc + c)
          ctx.lineTo(pxc + c, pyc + c)
        }
        if (!isOpen(s.maze, x, y, E)) {
          ctx.moveTo(pxc + c, pyc)
          ctx.lineTo(pxc + c, pyc + c)
        }
        ctx.stroke()
      }
  ...
  private drawSparksOutsideLight(s: GameState, x0: number, y0: number, x1: number, y1: number, t: number) {
  ...
  private drawPortal(s: GameState, t: number) {
    const ctx = this.ctx
    const c = this.camera.cell
    const x = this.sx(s.portal.x + 0.5),
      y = this.sy(s.portal.y + 0.5)
  ...
  private drawPlayer(s: GameState, t: number) {
    const ctx = this.ctx
    const c = this.camera.cell
    const x = this.sx(s.px),
      y = this.sy(s.py)
