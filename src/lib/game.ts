...
export type GameEvent = 'step' | 'spark' | 'charge' | 'pulse' | 'hit' | 'portal' | 'death' | 'lowlight' | 'levelstart'
...
export interface Wisp {
  px: number
  py: number // continuous position (cell units)
  fx: number
  fy: number // from cell
  tx: number
  ty: number // to cell
...
export interface GameState {
  ...
  // Player
  px: number
  py: number // continuous, cell units (center of cell = +0.5)
  cx: number
  cy: number // current cell
  moving: boolean
...
export class Game {
  ...
  private buildLevel(mode: Mode, seedLabel: string, level: number, carryScore: number, carryTime: number): GameState {
  ...
  nextLevel() {
    const s = this.state
    this.state = this.buildLevel(s.mode, s.seedLabel, s.level + 1, s.score, s.totalTime + s.time)
  ...
  update(dt: number) {
    ...
    // Portal.
    {
      const dx = s.portal.x + 0.5 - s.px,
        dy = s.portal.y + 0.5 - s.py
      if (dx * dx + dy * dy < 0.22) {
    ...
    for (const w of s.wisps) {
      ...
      while (remaining > 0) {
        if (w.t >= 1) {
          ...
          if (hunting) {
            const better = options.find((d) => field[idx(s.maze, w.fx + d.dx, w.fy + d.dy)] < myDist)
            if (better) choice = better
          } else if (options.length > 1) {
            const nonReverse = options.filter((d) => !(w.fx + d.dx === w.fx - (w.tx - w.fx) && w.fy + d.dy === w.fy - (w.ty - w.fy)))
