...
export const inBounds = (m: Maze, x: number, y: number) => x >= 0 && y >= 0 && x < m.cols && y < m.rows
export const isOpen = (m: Maze, x: number, y: number, bit: number) => (m.cells[idx(m, x, y)] & bit) !== 0
...
function generateBacktracker(m: Maze, rng: Rng) {
  ...
  while (stack.length) {
    ...
    const options = DIRS.filter((d) => inBounds(m, x + d.dx, y + d.dy) && !visited[idx(m, x + d.dx, y + d.dy)])
    if (!options.length) {
      stack.pop()
      continue
    }
...
function braid(m: Maze, rng: Rng, fraction: number) {
  ...
  for (let i = 0; i < target; i++) {
    ...
    const walls = DIRS.filter((d) => inBounds(m, x + d.dx, y + d.dy) && !isOpen(m, x, y, d.bit))
    if (walls.length) carve(m, x, y, walls[Math.floor(rng() * walls.length)])
  }
}

export function generateMaze(cols: number, rows: number, rng: Rng, algorithm: Algorithm, braidFraction = 0): Maze {
...
export function wallSegments(
  ...
  x0 = Math.max(0, x0)
  y0 = Math.max(0, y0)
  x1 = Math.min(m.cols - 1, x1)
  y1 = Math.min(m.rows - 1, y1)
  ...
  const hWall = (x: number, y: number) =>
    y === 0 ? !isOpen(m, x, 0, N) : y === m.rows ? !isOpen(m, x, m.rows - 1, S) : !isOpen(m, x, y, N)
  ...
  const vWall = (x: number, y: number) =>
    x === 0 ? !isOpen(m, 0, y, W) : x === m.cols ? !isOpen(m, m.cols - 1, y, E) : !isOpen(m, x, y, W)
  for (let x = x0; x <= x1 + 1; x++) {
    let runStart = -1
    for (let y = y0; y <= y1 + 1; y++) {
